...
describe("CLI-dispatcher adapters keep their dispatcher form (#738 non-regression)", () => {
  ...
  // `{ type: "command", command: "..." }` — NOT the nested Claude-Code
  // `{ matcher, hooks: [...] }` shape (verified against the official hooks
  // schema + the committed configs/{vscode,jetbrains}-copilot/hooks.json
  // templates; the nested shape never fires on Copilot). Read the command off
  // the flat entry, tolerating the nested shape for any future adapter.
  const flatCommands = (config: Record<string, unknown>): string[] =>
    Object.values(config).flatMap((arr) =>
      (arr as Array<{ command?: string; hooks?: Array<{ command: string }> }>).flatMap(
        (e) => (e.hooks ? e.hooks.map((h) => h.command) : e.command ? [e.command] : []),
      ),
    );
